...
const inputList = document.querySelector('.scheduleList');
...
function insertSchedule(text) {
  let scheduleObj = {
    schedule: text,
  }
  if (!DATA[currentDate]) {
    DATA[currentDate] = [];
    DATA[currentDate].push(scheduleObj);
  } else {
    DATA[currentDate].push(scheduleObj);
  }
  ...
  scheduleObj.id = DATA[currentDate].length;
  save();
  inputBox.value = '';
}
...
function redrawLi() {
  ...
  for (let scheduleList in DATA) {
    if (scheduleList === currentDate) {
      for (let i = 0; i < DATA[scheduleList].length; i++) {
        ...
        spanEl2.innerHTML = DATA[scheduleList][i].schedule;
        liEl2.appendChild(spanEl2);
        liEl2.appendChild(delBtn2);
        inputList.appendChild(liEl2);
        liEl2.setAttribute('id', DATA[scheduleList][i].id);
...
function resetInsert() {
  ...
  if (storeObj !== null) {
    ...
    parsed.forEach(function (schedule) {
      if (schedule) {
        ...
        lili.setAttribute('id', schedule.id);
        spanspan.innerHTML = schedule.schedule;
...
function delWork(e) {
  ...
  const cleanToDos = DATA[currentDate].filter(function (schedule) {
    return schedule.id !== parseInt(delParentLi.id);
  });
