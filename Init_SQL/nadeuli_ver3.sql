...
CREATE TABLE USER_CALENDER (
    `MEM_NO` BIGINT NULL,               -- not null
    `T_CONTENTNO` BIGINT NULL,         -- not null
    `CAL_NO` BIGINT AUTO_INCREMENT PRIMARY KEY,
    `CAL_COLOR` VARCHAR(100),             
    `CAL_TITLE` VARCHAR(100) NOT NULL,
    `CAL_STARTDATE` DATETIME NOT NULL,        -- <-Date
    `CAL_ENDDATE` DATETIME NOT NULL,          -- <-Date
    `CAL_MEMO` VARCHAR(100),             
    `CAL_BOARDNO` varchar(1000)
    -- ,FOREIGN KEY (MEM_NO) REFERENCES MEMBER(MEM_NO),
    -- FOREIGN KEY (T_CONTENTNO) REFERENCES TRAVEL(T_CONTENTNO)
)ENGINE = InnoDB
